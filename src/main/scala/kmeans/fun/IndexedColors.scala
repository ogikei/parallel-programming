...
class IndexedColorFilter(initialImage: Img,
  ...
  val points: GenSeq[Point] = imageToPoints(initialImage)
  val means: GenSeq[Point] = initializeIndex(colorCount, points)
  ...
  def getResult: Img = indexedImage(initialImage, newMeans)
